...
> **Status:** legado histórico. Não é contrato operacional do fluxo atual; consulte `AGENTS.md`, `docs/fluxo-operacao.md` e as skills canônicas (`run-editorial-batch`, `qa-draft` e `publicar-linkedin`).
...
- NÃO usar: para editar código do pipeline, para executar o fluxo editorial atual ou para tarefas não editoriais.
...
- Pastas (modelo histórico): `research/`, `content/ideas/`, `content/drafts/`, `content/published/`.
...
- "Publica um post aprovado" → no fluxo atual, invoque explicitamente `publicar-linkedin` para um arquivo em `content/drafts/` com marco `approved`.
...
- No fluxo atual, publicação/agendamento só ocorre no Bloco 3 e move o arquivo após confirmação.
