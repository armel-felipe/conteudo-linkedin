...
Cada stage/ciclo usa a chave idempotente `(run_id, topic_id, stage, cycle)`. Se essa chave já tiver commit canônico, artefato, resultado, review e checkpoint válidos, não execute o stage novamente; qualquer divergência de payload, fingerprint, caminho ou evento bloqueia. Uma gravação repetida do mesmo resultado não cria evento, review ou arquivo duplicado nem altera a ordem da fila. O `intent` inclui e valida os paths, `result` e `review` do payload recebido; nunca autoriza sobrescrever um ciclo parcial divergente.
